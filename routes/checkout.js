...
router.get('/', async (req, res) => {
  ...
  if('visaID' in sessionData) {
    ...
    try {
      ...
      const reqInstallments = await fetch(`${baseURL}/payment/installments`, {
        method: "POST",
        headers: {
...
router.post('/process-payment', async (req, res) => {
  ...
  try {
    ...
    const newOrder = await fetch(`${baseURL}/order`, {
      method: "POST",
    ...
    const createPayment = await fetch(`${baseURL}/payment/credit-card`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        "access-token": process.env.APPMAX_ACCESS_TOKEN,
        ...
        "payment": {
          "CreditCard": {
            ...
            "name": cardholderName,
            "installments": separateInstallmentsAndValues(installmentsInput),
...
router.post("/process-payment-pix", async (req, res) => {
  ...
  try {
    ...
    const createPayment = await fetch(`${baseURL}/payment/pix`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        "access-token": process.env.APPMAX_ACCESS_TOKEN,
...
router.post('/webhooks', async (req, res, next) => {
  const { body } = req
  const paymentResponse = Payment.findOne({idOrder: body.data.id})
